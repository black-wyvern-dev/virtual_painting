...
define(function() {
  ...
  Layer.prototype.setAlpha = function (alpha) {
    ...
    for (var i = 3; i < data.length; i += 4) {
      if(data[i-3] == 255 && data[i-2] == 255 && data[i-1] == 255) data[i] = 0;
      else 
      data[i] = alpha;
    }
    return this;
  ...
  Layer.prototype.fill = function (rgba) {
    ...
    for (var i = 0; i < data.length; i += 4)
      ...
      if(j==3 && data[i + j-3] == 255 && data[i + j-2] == 255 && data[i + j-1] == 255) data[i + j] = 0;
      else 
        data[i + j] = rgba[j];
    return this;
  };
  ...
  Layer.prototype.convertGrayImage = function () {
    var data = this.imageData.data, lightness;
    for (var i = 0; i < data.length / 4; i++) {
      lightness = Math.ceil((data[4 * i] + data[4 * i + 1] + data[4 * i + 2]) / 3);
      data[4 * i] = lightness;
      data[4 * i + 1] = lightness;
      data[4 * i + 2] = lightness;
      // data[4 * i + 3] = 0;
    }
  };

  Layer.prototype.setGrayAlpha = function (offset, isShow) {
    if (offset == undefined) {
      this.setAlpha(0);
      return;
    }

    if (isShow) this.imageData.data[offset + 3] = 255;
    else  this.imageData.data[offset + 3] = 0;
  }
